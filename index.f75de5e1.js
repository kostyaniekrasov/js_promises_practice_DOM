...
const thirdPromise = new Promise((resolve)=>{
    ...
    logo.addEventListener("mousedown", (e)=>{
        if (e.button === 0) leftCliked = true;
        else if (e.button === 2) rightCliked = true;
        if (leftCliked && rightCliked) resolve("Third promise was resolved");
    });
});
firstPromise.then((message)=>pushNotification(message, "success")).catch((error)=>pushNotification(error.message, "error"));
